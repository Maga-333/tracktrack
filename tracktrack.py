...
ALLOWED_USER = 'superstar'  # your Linux username

if getpass.getuser() != ALLOWED_USER:
    print("⛔ Unauthorized user. Access denied.")
    sys.exit()

# Step 1: Base command database (we’ll expand in Step 3+)
command_database = {
    "rm": {
...
write_commands = [
    ...
    "cut", "sort", "uniq", "wc", "diff", "comm"  # hypothetical for completeness
]

# ✅ Step 2: Log the command
def log_command(cmd):
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    with open("logs/command_log.txt", "a") as f:
        f.write(f"[{timestamp}] {cmd}\n")

# ✅ Step 3: Check the command + option for danger
